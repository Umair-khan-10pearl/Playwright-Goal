...
test.describe('Authentication - Login', () => {
  ...
  test('User cannot login with invalid username @negative @authentication', async ({ loginPage }) => {
      await loginPage.login('invaliduser@gmail.com',ENV.password,);
      expect(await loginPage.loginErrorMessage()).toBeVisible();
    },
  );

  test('User cannot login with invalid password @negative @authentication', async ({ loginPage }) => {
